...
const GameCard: React.FC<GameCardProps> = ({ title, ign, id, platform }) => {
  ...
  return (
    <Card className="group relative overflow-hidden bg-gradient-to-br from-slate-800/90 to-slate-900/90 border-2 border-cyan-400/30 hover:border-cyan-400 transition-all duration-500 transform hover:scale-105 hover:shadow-2xl hover:shadow-cyan-400/30 retro-card">
      ...
      <CardHeader className="pb-3 relative z-10">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-bold text-white group-hover:text-cyan-400 transition-colors duration-300 retro-text">
            {title}
          </CardTitle>
          <span className={`px-3 py-1 text-xs font-bold rounded-full border-2 transition-all duration-300 ${
            platform === 'Mobile' 
      ...
      <CardContent className="relative z-10">
        <div className="space-y-3">
          <div className="flex items-center justify-between group/item">
            <span className="text-gray-400 text-sm font-medium">IGN/Nickname:</span>
            <div className="flex items-center gap-2">
              <span className="text-cyan-400 font-bold">{ign}</span>
              <button
                onClick={() => copyToClipboard(ign, 'IGN')}
                className="opacity-0 group-hover/item:opacity-100 text-gray-400 hover:text-cyan-400 transition-all duration-200 hover:scale-110"
              >
                <Copy size={14} />
              </button>
            </div>
          ...
          <div className="flex items-center justify-between group/item">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="opacity-0 group-hover/item:opacity-100 text-gray-400 hover:text-purple-400 transition-all duration-200 hover:scale-110"
              >
                <Copy size={14} />
              </button>
            </div>
